...
-type redis_error_result() :: Reason :: bitstring() | no_connection | invalid_cluster_command.
-type redis_success_result() :: Result :: bitstring().
-type redis_simple_result() :: {ok, redis_success_result()} | {error, redis_error_result()}.
-type redis_pipeline_result() :: [redis_simple_result()].
-type redis_transaction_result() :: {ok, [redis_success_result()]} | {error, redis_error_result()}.
-type redis_result() :: redis_simple_result() | redis_pipeline_result().

-record(node, {address  :: string(),
               port     :: integer(),
               pool     :: atom()}).

-record(slots_map, {start_slot  :: integer(),
                    end_slot    :: integer(),
                    index       :: integer(),
                    node        :: #node{}}).
